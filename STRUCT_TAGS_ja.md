...
> **コード生成に関する注意:** 静的コードジェネレータは `count(F)` と、ほぼすべてのフィールド形状に対する `bytelen(F)`（スカラー・スカラー配列、バイトスライス・配列、テキストエンコードを含む全文字列バリアント、ネスト構造体、タグ計数の構造体配列、構造体ポインタ）をサポートします。**カスタム `valueof` 評価関数**は、参照するフィールドがすべて*バイト領域*フィールド（`[]byte`/`[N]byte`、生の `string`、テキストエンコーディングなしの定数長 `string(N)`）または*固定幅の整数スカラー*（`uint8`〜`uint64`、符号付き、`byte`/`word`/`dword`/`qword`）の場合にサポートされます。それ以外の形状（テキストエンコードされた文字列、プレフィックス／終端付き文字列、ネスト構造体、浮動小数点、スカラー配列）は生成時にエラーになります（ランタイムを使用してください）。生成コードは非 nil の Marshaler が必要なため `WriteBinaryWithMarshaler` を使用します。デコード時の検証は**デフォルトで有効**で、生成されたデコードは値を再計算して検証し、ランタイムインタプリタと一致します。`-no-validate` フラグはすべてのデコード時検証（このカスタムチェックに加えて組み込みの `const`/`range`/`match` チェック）を取り除きます。
