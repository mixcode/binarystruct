...
- **Codegen support for custom `valueof` evaluators** when every referenced field
  is a *byte-region* field (`[]byte`/`[N]byte`, raw `string`, or constant-size
  `string(N)` without text encoding) — those have no byte-order/encoding transform,
  so the generated bytes match the runtime exactly. Generated code requires a
  non-nil Marshaler (like `codec=`; call `WriteBinaryWithMarshaler`). Decode-time
  validation is opt-in via the new **`-valueof-validate`** codegen flag (default off
  / encode-only).

### Limitations
- Codegen rejects a custom `valueof` evaluator whose referenced arg is **not** a
  byte-region field (text-encoded string, multibyte scalar, prefixed/terminated
  string, nested struct) — generation fails loud; use the runtime interpreter for
  those structs. (Alongside the existing codegen exclusions: struct-level
  `endian=inverse` and order/encoding inheritance via embedding.)
