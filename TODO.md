...
- [ ] **Codegen multidimensional arrays (deferred)**: the runtime now supports multidimensional array tags, but `binarystruct-codegen` fails loud on them. Generating them needs nested loops plus nested `make()` on decode (scoped to scalar/byte leaves first). Low priority — the runtime interpreter handles them.
