...
* **コード生成** `-unsafe-bulk`（任意）: Go要素の幅がワイヤ上の幅と一致する固定幅スカラー配列・スライスについて、デフォルトの要素ごとのループの代わりに、生メモリの一括処理パス（1回の `Write`/`ReadFull` ＋ 1回のインプレースなバイトスワップ。amd64 で `-tags experiment_simd` を付けるとSIMDで高速化）を生成できます。出力バイトは同一で、移植性と引き換えに速度を得るだけです。デフォルトは無効。
