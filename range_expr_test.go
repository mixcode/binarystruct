...
func TestRange_HexBounds(t *testing.T) {
	...
	mustPass := func(val uint8) {
		blob, err := Marshal(&S{V: val}, BigEndian)
		if err != nil {
	...
	mustFail := func(val uint8) {
		...
		var out S
		_, err = Unmarshal(blob, BigEndian, &out)
...
func TestRange_ArithmeticBounds(t *testing.T) {
	...
	var out S
	good, _ := Marshal(&S{V: 100}, BigEndian)
	if _, err := Unmarshal(good, BigEndian, &out); err != nil {
		t.Fatalf("100 rejected: %v", err)
	}
	bad, _ := Marshal(&S{V: 101}, BigEndian)
	if _, err := Unmarshal(bad, BigEndian, &out); !errors.Is(err, ErrValidationError) {
...
func TestRange_FloatBounds(t *testing.T) {
	...
	var out S
	good, _ := Marshal(&S{F: 2.0}, BigEndian)
	if _, err := Unmarshal(good, BigEndian, &out); err != nil {
		t.Fatalf("2.0 rejected: %v", err)
	}
	bad, _ := Marshal(&S{F: 9.0}, BigEndian)
	if _, err := Unmarshal(bad, BigEndian, &out); !errors.Is(err, ErrValidationError) {
...
func TestRange_OpenHexBound(t *testing.T) {
	...
	var out S
	good, _ := Marshal(&S{V: 0x200}, BigEndian)
	if _, err := Unmarshal(good, BigEndian, &out); err != nil {
		t.Fatalf("0x200 rejected: %v", err)
	}
	bad, _ := Marshal(&S{V: 0xff}, BigEndian)
	if _, err := Unmarshal(bad, BigEndian, &out); !errors.Is(err, ErrValidationError) {
