...
- **Codegen support for custom `valueof` evaluators** when every referenced field
  ...
  `string(N)` without text encoding) or a *fixed-width integer scalar*
  (`uint8`…`uint64`, signed, `byte`/`word`/`dword`/`qword`, encoded with
  `order.PutUintN`). For these the generated bytes match the runtime exactly
  ...
  **on by default** — generated decode recomputes the value and, on mismatch,
  returns a `*DecodeError` wrapping `ErrValidationError` (offset + field),
...
- Codegen rejects a custom `valueof` evaluator whose referenced arg is neither a
  ...
  generation fails loud; use the runtime interpreter for those structs. (Alongside
  the existing codegen exclusions: struct-level `endian=inverse` and order/encoding
